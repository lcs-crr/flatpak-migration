...
do_phase_a() {
  ...
  for pkg in "${GUI_RPMS[@]}"; do
    ...
    if flatpak info "$appid" >/dev/null 2>&1; then
      # Flatpak already installed. With --auto-remove we still want the
      # RPM gone, so include it; the install step will be skipped at
      # execution time. Otherwise just report and move on.
  ...
  for e in "${FINAL[@]}"; do
    pkg="${e%|*}"; appid="${e##*|}"
    if flatpak info "$appid" >/dev/null 2>&1; then
      printf '   %s~%s %-35s -> %s %s(Flatpak present, RPM will be removed)%s\n' \
        "$c_yellow" "$c_reset" "$pkg" "$appid" "$c_dim" "$c_reset"
    else
      printf '   %s+%s %-35s -> %s\n' "$c_green" "$c_reset" "$pkg" "$appid"
    fi
  done
  ...
  for entry in "${FINAL[@]}"; do
    ...
    if flatpak info "$appid" >/dev/null 2>&1; then
      info "${c_bold}$appid${c_reset} already present as Flatpak"
    else
      info "${c_bold}Installing $appid${c_reset}  (replaces $pkg)"
    ...
    if [[ $installed_ok -eq 1 && $AUTO_REMOVE -eq 1 ]]; then
